...
iteration_shift = {0: 1, 1: 1, 2: 2, 3: 2, 4: 2, 5: 2, 6: 2, 7: 2, 8: 1, 9: 2, 10: 2, 11: 2, 12: 2, 13: 2, 14: 2, 15: 1}
...
def des_function(binary_input, key):
    ...
    expansion = ""
    # Expansion P-block
    for bit in expansion_bit:
        expansion += binary_input[bit - 1]
    # XOR with Key
    expansion = xor(expansion, key)
    # S-Boxes
    s_box = "".join([s_box_value(expansion[s_boxes.index(s_box) * 6: s_boxes.index(s_box) * 6 + 6], s_box)
                     for s_box in s_boxes])
    # Straight P-box
    for bit in straight_p_box:
        result += s_box[bit - 1]
    return result
...
def des(mode, text, key):
    result = ""
    keys = []
    bin_text = "".join(['{0:0{1}b}'.format(ord(char), 8) for char in text])
    bin_key = "".join(['{0:0{1}b}'.format(ord(char), 8) for char in key])
    p_text = ""
    pc1_key = ""
    # Initial Permutation for text and key: IP and pc1
    for bit in IP:
        p_text += bin_text[bit - 1]
    for bit in pc1:
        pc1_key += bin_key[bit - 1]
    # Split left and right for text and key
    left_text = p_text[:32]
    right_text = p_text[32:]
    left_key = pc1_key[:28]
    ...
    for des_round in range(16):
        ...
        left_right_key = left_key + right_key
        pc2_key = ""
        for bit in pc2:
            pc2_key += left_right_key[bit - 1]
        keys.append(pc2_key)
    # 16 rounds
    for des_round in range(16):
        # XOR
        if mode == 1:
            left_text = xor(left_text, des_function(right_text, keys[des_round]))
        else:
            left_text = xor(left_text, des_function(right_text, keys[15 - des_round]))
        # Swap left and right unless on the last round
        if des_round != 15:
            left_text, right_text = right_text, left_text
    # Final Permutation: IP_Inverse
    final = left_text + right_text
...
def main():
    ...
    while len(key) != 8:
        ...
        key = input("Please enter the key: ")
    # IV is not going to be implemented here as we are going to use ECB instead of other modes.
    # Pad if text is not a multiple of 8 (a char is a byte and 8 bytes is 64 bits)
    if len(text) % 8 != 0:
        text += ''.join(["0" for _ in range(8 - (len(text) % 8))])
    # DO NOT MODIFY #
    # Break into blocks
    result = ""
    for index in range(int(len(text) / 8)):
        result += des(mode, text[index * 8: index * 8 + 8], key)
    print("".join([chr(int(result[i * 8: i * 8 + 8], 2)) for i in range(int(len(result) / 8))]))
    print(result)
